...
def touch(request):
    def smart_touch(path, name):
        ...
        request.fs.create(request.fs.join(urllib_unquote(path.encode('utf-8')), urllib_unquote(name.encode('utf-8'))))
...
def move(request):
    ...
    def bulk_move(*args, **kwargs):
        for arg in args:
            ...
            request.fs.rename(urllib_unquote(arg['src_path'].encode('utf-8')), urllib_unquote(arg['dest_path'].encode('utf-8')))
